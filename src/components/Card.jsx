...
const Card = ({ poke }) => {
    ...
    return (
        <div className={`${poke.types[0].type.name} shadow-2xl size-60 p-5 m-5 text-center !bg-opacity-30 rounded-lg relative border isolate overflow-hidden`}>
            ...
            <h1 className="absolute text-9xl font-bold text-white opacity-20 z-0 left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2">{pokeId}</h1>
            <h1 className="absolute z-20 text-base right-0 top-0 m-3 drop-shadow-lg">{pokeId}</h1>
